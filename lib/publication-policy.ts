...
export function readyArticleFailures(
  ...
  if (!Number.isFinite(article.quality_score) || article.quality_score < AUTOMATIC_PUBLICATION_QUALITY_THRESHOLD) failures.push("quality score below 90");
...
export function selectBestReadyArticle(
  ...
  return articles.filter((article) => readyArticleFailures(article, permittedSourceIds, now).length === 0 &&
